...
def test_analysis_prompt_generation():
    ...
    for file_count, byte_count, expected_keywords in test_cases:
        ...
        keyword_found = any(keyword.lower() in prompt.lower() for keyword in expected_keywords)
        assert keyword_found, f"None of {expected_keywords} found in prompt for {file_count} files"
...
def test_security_path_validation():
    ...
    with patch("ollama.Client") as mock_client_cls:
        ...
        for path in dangerous_paths:
            ...
            assert ("system directories is not allowed" in result or 
                   "does not exist" in result or
                   "not accessible" in result or
                   "Invalid directory path" in result), f"Path {path} not properly protected"
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("ollama.Client") as mock_client_cls:
            ...
            if caps["files_truncated"] > 0:
                print(f"  Large file was properly truncated (files_truncated: {caps['files_truncated']})")
...
def test_chat_functionality():
    ...
    with patch("ollama.Client") as mock_client_cls:
        ...
        assert "This is a test codebase with Python files." in call_args.kwargs["prompt"]

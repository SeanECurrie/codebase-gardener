        ...
        for directory in directories:
            if directory:
                try:
                    ...
                    logger.error(
                        "Failed to create directory", path=str(directory), error=str(e)
                    )
    ...
    def _initialize_active_project_state(self) -> None:
        ...
        if not self.active_project_file.exists():
            ...
            try:
                ...
                raise DirectorySetupError(
                    f"Cannot create active project state: {e}"
                ) from e
        ...
        try:
            ...
            if self.active_project_file.exists():
                backup_path.write_text(
                    self.active_project_file.read_text(encoding="utf-8")
                )
            ...
            logger.debug(
                "Updated active project state", path=str(self.active_project_file)
            )

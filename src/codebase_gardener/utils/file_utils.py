...
class FileUtilities:
    ...
    def _recursive_scan_with_exclusions(
        self,
        dir_path: Path,
        patterns: list[str],
        include_hidden: bool,
    ...
    def _should_exclude_directory(
        self, dir_path: Path, exclude_patterns: list[str]
    ) -> bool:
    ...
    def find_source_files(
        ...
        try:
            ...
            for file_path in self.scan_directory(
                dir_path, recursive=True, exclude_patterns=exclusion_patterns
            ):
                # Enforce optional timeout
                if timeout is not None and (_time.time() - start_time) >= timeout:
                    if progress_callback:
                        progress_callback(
                            "⏰ File discovery timeout reached, aborting scan"
                        )
                ...
                if progress_callback and files_processed % 50 == 0:
                    progress_callback(
                        f"Processed {files_processed} files, found {len(source_files)} source files"
                    )
            ...
            if progress_callback:
                progress_callback(
                    f"✅ Completed: found {len(source_files)} source files in {files_processed} total files"
                )

            logger.info(
                f"File discovery completed: found {len(source_files)} source files"
            )
            return source_files
            ...
            raise FileUtilityError(
                f"Could not discover files in {dir_path}: {e}"
            ) from e
    ...
    def _matches_file_exclusion_patterns(
            ...
            for file_path in self.scan_directory(dir_path, recursive=True):
                try:
                    ...
                    if stat_info.st_mtime > since_timestamp:
                        change_type = (
                            "modified"
                            if stat_info.st_ctime < since_timestamp
                            else "created"
                        )
                        changes.append(
                            FileChange(
    ...
    def compare_snapshots(
        self, old: FileSnapshot, new: FileSnapshot
    ) -> list[FileChange]:
        """
...
def find_source_files(
    dir_path: Path, languages: list[str] | None = None, progress_callback=None
) -> list[Path]:
    """Find source code files in directory with progress feedback."""
    return file_utilities.find_source_files(
        dir_path, languages, progress_callback=progress_callback
    )

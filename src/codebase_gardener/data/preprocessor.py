...
from codebase_gardener.utils.error_handling import PreprocessingError, retry_with_backoff
...
class CodePreprocessor:
    ...
    def _generate_chunks(self, parse_result: ParseResult, code: str, file_path: Optional[Path]) -> List[CodeChunk]:
        ...
        for element in parse_result.structure.get_all_elements():
            chunk = self._create_chunk_from_element(element, code, file_path, module_info)
        ...
        module_chunk = self._create_module_chunk(parse_result, code, file_path, module_info)
    ...
    def _create_chunk_from_element(
        ...
        try:
            ...
            complexity = self._calculate_complexity(element, code) if self.config.calculate_complexity else 0.0
    ...
    def _extract_dependencies(self, element: CodeElement, module_info: Dict[str, Any]) -> List[str]:
        ...
        keywords = {
            'if', 'else', 'for', 'while', 'def', 'class', 'return', 'len', 'str', 'int', 'float',
            'self', 'True', 'False', 'None', 'and', 'or', 'not', 'in', 'is', 'with', 'as',
            'try', 'except', 'finally', 'raise', 'from', 'import', 'pass', 'break', 'continue'
        }
        dependencies = list(set(dep for dep in dependencies if dep not in keywords and len(dep) > 1))
    ...
    def _split_chunk(self, chunk: CodeChunk, code: str) -> List[CodeChunk]:
        ...
        for i in range(max(1, mid_point - 5), min(len(lines) - 1, mid_point + 5)):
            line = lines[i].strip()
            if not line or line.startswith('#') or line.startswith('def ') or line.startswith('class '):
                break_point = i
                break
    ...
    def _add_chunk_overlap(self, chunks: List[CodeChunk]) -> List[CodeChunk]:
        ...
        for i, chunk in enumerate(chunks):
            ...
            overlap_lines = min(
                self.config.overlap_size // 50,  # Approximate lines from character count

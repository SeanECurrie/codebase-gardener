...
class TreeSitterParser:
    ...
    def __init__(self, language: str | SupportedLanguage | None = None, settings=None):
        """
        Initialize parser for a specific language.

        Args:
            language: Programming language to parse (optional for component registry)
        ...
        """
        if language is None:
            # For component registry initialization - will need to call setup_language later
            self.language = None
    ...
    def setup_language(self, language: str | SupportedLanguage) -> None:
        """
        Set up language for a parser initialized without language.
    ...
    def _setup_language(self) -> None:
        ...
        if self.language is None:
            # Can't set up without a language
            return
        ...
        try:
            if self.language == SupportedLanguage.PYTHON:
                ...
                raise ParsingError(
                    f"Language setup not implemented: {self.language.value}"
                )

            # Initialize parser with language (new API)
            self._parser = Parser(self._language_obj)
    ...
    def parse(
        self, code: str, file_path: Path | None = None, old_tree: Tree | None = None
    ) -> ParseResult:
        """
        ...
        try:
            ...
            if self._parser is None:
                raise ParsingError(
                    "Parser not initialized - call setup_language() first",
                    details={
                        "language": self.language.value if self.language else "none"
                    },
                )
            ...
            if old_tree is not None:
                tree = self._parser.parse(code_bytes, old_tree)
            else:
        ...
        def traverse_node(node: Node):
            ...
            if node.type in element_types:
                element = self._create_code_element(
                    node, code, element_types[node.type]
                )
                if element:
    ...
    def _create_code_element(
        self, node: Node, code: str, element_type: str
    ) -> CodeElement | None:

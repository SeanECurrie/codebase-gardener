...
class ProjectMetadata(BaseModel):
    ...
    training_status: TrainingStatus = Field(
        default=TrainingStatus.NOT_STARTED, description="Current training status"
    )
    ...
    lora_adapter_path: Path | None = Field(
        None, description="Path to the trained LoRA adapter"
    )
    vector_store_path: Path | None = Field(
        None, description="Path to the project's vector store"
    )
    context_path: Path | None = Field(
        None, description="Path to conversation context file"
    )
...
class RegistryData(BaseModel):
    ...
    projects: dict[str, ProjectMetadata] = Field(
        default_factory=dict, description="Map of project_id to metadata"
    )
...
class ProjectRegistry:
    ...
    def _load_registry(self) -> None:
        ...
        try:
            ...
            if "projects" in data:
                for project_data in data["projects"].values():
                    ...
                    if "last_updated" in project_data:
                        project_data["last_updated"] = datetime.fromisoformat(
            ...
            if self.registry_file.exists():
                ...
                logger.warning(
                    "Corrupted registry backed up", backup_file=str(backup_file)
                )
    ...
    def _save_registry(self) -> None:
        ...
        try:
            ...
            logger.debug(
                "Registry saved successfully", registry_file=str(self.registry_file)
            )
    ...
    def register_project(
        self, name: str, source_path: Path, language: str = "python"
    ) -> str:
        """
        ...
        with self._lock:
            ...
            if project_dir.exists():
                ...
                try:
                    ...
                    logger.debug(
                        "Project directory removed", project_dir=str(project_dir)
                    )
                except OSError as e:
            ...
            if self._data.active_project == project_id:
                ...
                self._data.active_project = (
                    remaining_projects[0] if remaining_projects else None
                )

            self._save_registry()

            logger.info(
                "Project removed successfully", project_id=project_id, name=project_name
            )
    ...
    def validate_registry(self) -> list[str]:
        ...
        if (
            self._data.active_project
            and self._data.active_project not in self._data.projects
        ):

...
class Settings(BaseSettings):
    ...
    lora_rank: int = Field(
        default=16,
        description="LoRA adapter rank for fine-tuning",
        ge=1,
        le=64
    )
    lora_alpha: int = Field(
        default=32,
        description="LoRA alpha parameter for fine-tuning",
        ge=1,
        le=128
    )
    training_batch_size: int = Field(
        default=4,
        description="Batch size for LoRA training",
        ge=1,
        le=16
    )
    ...
    gradio_host: str = Field(
        default="127.0.0.1",
        description="Host for Gradio web interface"
    )

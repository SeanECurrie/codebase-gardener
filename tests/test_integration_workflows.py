...
""")
        ...
        for i in range(15):
            (base_dir / "src" / f"module_{i}.py").write_text(f"""
...
""")
        ...
        for i in range(10):
            (base_dir / "tests" / f"test_module_{i}.py").write_text(f"""
\"\"\"Tests for module {i}.\"\"\"
import unittest
...
""")
        ...
        (base_dir / "config" / "settings.py").write_text("DEBUG = True\nLOG_LEVEL = 'INFO'")
        ...
        for subdir in ["lib", "tools", "scripts", "docs"]:
            ...
            for i in range(30):
                (base_dir / subdir / f"{subdir}_file_{i}.py").write_text(f"""
# {subdir.title()} file {i}
...
def test_complete_analysis_workflow_medium_project():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("ollama.Client") as mock_client_cls:
            ...
            assert has_src or has_tests or has_config  # Should detect organized structure
...
def test_multiple_chat_interactions():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("ollama.Client") as mock_client_cls:
            ...
            for i, (question, expected_response) in enumerate(zip(questions, chat_responses)):
...
def test_environment_configuration_workflow():
    ...
    auditor3 = CodebaseAuditor(
        ollama_host="http://explicit-host:11434",
...
def test_large_project_handling_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("ollama.Client") as mock_client_cls:
            ...
            result = auditor.analyze_codebase(str(test_dir), progress_callback=progress_callback)
            ...
            assert caps["files_skipped"] > 0 or caps["files_truncated"] > 0  # Some limiting occurred
...
def test_mixed_file_types_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        (test_dir / "frontend.js").write_text("// JavaScript frontend\nfunction render() {}")

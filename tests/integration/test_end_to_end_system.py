...
""",
    ...
    def app_context(self, mock_settings):
        ...
        with patch(
            "src.codebase_gardener.config.settings.get_settings",
    ...
    def test_complete_project_lifecycle(self, app_context, sample_project_dir):
        ...
        with patch.object(
            app_context.dynamic_model_loader, "train_project"
        ) as mock_train:
            mock_train.return_value = True

            training_success = app_context.dynamic_model_loader.train_project(
                project_id
            )
        ...
        with patch.object(
            app_context.dynamic_model_loader, "generate_response"
        ) as mock_generate:
            mock_generate.return_value = (
                "This is a simple test function that returns a string."
            )

            # Mock vector store operations
            with patch.object(
                app_context.vector_store_manager, "get_project_vector_store"
            ) as mock_get_store:
                ...
                with patch(
                    "src.codebase_gardener.models.nomic_embedder.get_nomic_embedder"
                ) as mock_embedder:
                    mock_embedder_instance = Mock()
                    mock_embedder_instance.embed_code.return_value = [
                        0.1,
                        0.2,
                        0.3,
                    ]  # Mock embedding
        ...
        with patch.object(
            app_context.project_registry, "remove_project"
        ) as mock_remove:
    ...
    def test_multi_project_workflow(self, app_context, sample_project_dir):
        ...
        with patch.object(
            app_context.context_manager, "switch_project"
        ) as mock_context_switch:
    ...
    def test_error_recovery_and_graceful_degradation(
        self, app_context, sample_project_dir
    ):
        ...
        with patch.object(
            app_context.dynamic_model_loader, "switch_project"
        ) as mock_switch:
            mock_switch.side_effect = Exception("Model loader error")

            # System should continue despite model loader failure
            with patch.object(
                app_context.context_manager, "switch_project", return_value=True
            ):
                with patch.object(
                    app_context.vector_store_manager,
                    "switch_project",
                    return_value=True,
                ):
                    ...
                    try:
                        ...
                        pytest.fail(
                            f"System should handle model loader errors gracefully: {e}"
                        )
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                try:
                    ...
                    assert analysis_result[
                        "ai_response"
                    ], "AI analysis should work without vector store"
                except Exception as e:
                    pytest.fail(
                        f"Analysis should work despite vector store failure: {e}"
                    )

        # Scenario 3: Context manager failure
        with patch.object(
            app_context.context_manager, "add_message"
        ) as mock_add_message:
            ...
            try:
                ...
                pytest.fail(
                    f"Chat should handle context manager errors gracefully: {e}"
                )
    ...
    def test_performance_benchmarks(self, app_context, sample_project_dir):
        ...
        assert (
            init_time < 5.0
        ), f"Initialization took {init_time:.2f}s, should be under 5s"
        ...
        with patch.object(app_context, "switch_project") as mock_switch:
            ...
            assert (
                switch_time < 3.0
            ), f"Project switch took {switch_time:.2f}s, should be under 3s"
        ...
        assert (
            memory_usage < 500
        ), f"Memory usage {memory_usage:.1f}MB should be under 500MB"
        ...
        assert (
            concurrent_time < 2.0
        ), f"Concurrent operations took {concurrent_time:.2f}s, should be under 2s"
    ...
    def test_system_health_monitoring(self, app_context):
        ...
        for component in expected_components:
            ...
            assert (
                "initialized" in components[component]
            ), f"{component} should have initialized flag"
        ...
        if "integration_health" in health_status:
            ...
            assert (
                "score" in integration_health
            ), "Integration health should include score"
            assert (
                "details" in integration_health
            ), "Integration health should include details"
    ...
    def test_data_isolation_between_projects(self, app_context, sample_project_dir):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                assert (
                    mock_context_switch.call_count == 2
                ), "Should switch context twice"
                assert (
                    mock_add_message.call_count == 2
                ), "Should add messages to separate contexts"
        ...
        with patch.object(
            ...
            store_1 = app_context.vector_store_manager.get_project_vector_store(
                project_1_id
            )
            store_2 = app_context.vector_store_manager.get_project_vector_store(
                project_2_id
            )
            ...
            ), "Should request separate vector stores"

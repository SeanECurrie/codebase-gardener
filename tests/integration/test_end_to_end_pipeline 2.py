...
class TestEndToEndPipeline:
    ...
    def test_complete_pipeline_with_real_codebase(
        self, real_settings, real_codebase_files
    ):
        ...
        assert (
            processing_time < 300
        ), f"Processing too slow: {processing_time:.1f}s"  # 5 min max
        assert (
            memory_used < 2000
        ), f"Memory usage too high: {memory_used:.1f}MB"  # 2GB max
    ...
    def test_cache_persistence_with_real_files(
        self, real_settings, real_codebase_files
    ):
        ...
        print(
            f"    📊 Cache entries: {cache_stats['memory_entries']} memory, {cache_stats['disk_entries']} disk"
        )

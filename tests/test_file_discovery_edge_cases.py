...
def test_empty_and_invalid_directories():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        only_excluded.mkdir()
        (only_excluded / "README.txt").write_text(
            "Not a source file"
        )  # .txt not in SOURCE_CODE_EXTENSIONS
...
def test_language_filtering():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        multi_lang_files = utils.find_source_files(
            test_dir, languages=["python", "java"]
        )
...
def test_custom_exclusion_patterns():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        files_custom = utils.find_source_files(
            test_dir, exclude_patterns=["custom_exclude", "temp"]
        )
...
def test_progress_callback_detail():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        utils.find_source_files(test_dir, progress_callback=detailed_progress)
        ...
        scanning_msgs = [
            msg for msg in progress_messages if "Scanning directory" in msg
        ]
        assert len(scanning_msgs) >= 1

        progress_msgs = [
            msg for msg in progress_messages if "Processed" in msg and "files" in msg
        ]

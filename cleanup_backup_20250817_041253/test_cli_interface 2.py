...
def test_cli_command_parsing():
    ...
    from codebase_auditor import print_help, print_welcome
...
def test_security_command_injection():
    ...
    for injection in injection_attempts:
        ...
        assert (
            ...
            or "system directories" in result
        )
...
def test_large_project_performance():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Create source files
        for i in range(50):
            (test_dir / f"module_{i:02d}.py").write_text(
                f"""
# Module {i}
class Module{i}:
    def __init__(self):
...
"""
        ...
        for subdir in ["utils", "models", "tests", "config"]:
            ...
            sub_path.mkdir()
            for j in range(10):
                (sub_path / f"{subdir}_{j}.py").write_text(
                    f"""
# {subdir} file {j}
...
"""
        ...
        with patch("ollama.Client") as mock_client_cls:
            ...
            mock_client.generate.return_value = {
                "response": "Performance test analysis"
            }
